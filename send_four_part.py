...
【ポイント】
重要点を3〜5個の箇条書き。「- 」で開始。各項目は必ず以下を含む：
1. 具体的な数値データ（価格、性能、ユーザー数、日付など）
2. 固有名詞（製品名、会社名、技術名など）
3. 実際の影響や変化（何が変わるか、誰に影響するか）
単なる機能説明ではなく「なぜ重要か」が分かる内容にする。各項目80-120字で情報密度を高める。
【用語解説】
「ポイント」に登場した専門用語のみを「- 用語名：一言説明」で解説。該当がない場合は記事の主要キーワード5個以上選び同形式で補足。

制約：
- 全体1000文字以内（文字数配分はポイントに厚く、やさしい要約は簡潔に）
- エンジニア以外も理解できる表現
...
def main() -> int:
    ...
    try:
        ...
        if title_ja and points:
            # scoring to derive importance label (2-level: 全社共有 / 特定部署向け)
            score, _, _ = score_article(title_en, content_en, published)
            score_label = "全社共有" if score >= 5 else "特定部署向け"
